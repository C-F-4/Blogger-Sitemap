var tabbedSM_defaults = {
	blogUrl: "https://codersfabric.blogspot.in/", // Blog URL
	containerId: "tabbed-sm",                     // Container ID
	...
	newTabLink: true,                             // true if Open link in new window
...
for (var i in tabbedSM_defaults) {
	tabbedSM_defaults[i] = (typeof(tabbedSM[i]) !== undefined && typeof(tabbedSM[i]) !== 'undefined') ? tabbedSM[i] : tabbedSM_defaults[i];
}

function clickTab(pos) {
	var a = document.getElementById(tabbedSM_defaults.containerId),
...
(function() {
	var h = document.getElementsByTagName('head')[0],
		...
		s.src = tabbedSM_defaults.blogUrl + '/feeds/posts/summary?alt=json-in-script&max-results=' + tabbedSM_defaults.maxResults + '&orderby=published&callback=showTabs';
	if (tabbedSM_defaults.preload !== "onload") {
		setTimeout(function() {
			h.appendChild(s);
		}, tabbedSM_defaults.preload);
	} else {
